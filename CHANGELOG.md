...
- **Consolidation freeze of the Reg CF funding-portal control set (holistic audit)** — after building all 9 controls across 4 sovereigns incrementally (each diff adversarially verified), a whole-artifact audit (not per-diff) confirmed the set is holistically sound — consistent verdict semantics, `satisfied` reached only by the one pure two-public-register control (`portal_finra_member`), no never-opine leaks across the boundary cases (`cf_state_notice` law-citation, `portal_conduct` compensation, the `satisfied` note), all authority nodes resolve with 0 coverage gaps, defensive ctx/fd contracts. It found one real cross-cutting gap incremental review missed: the **Form C driver of the entire framework had no Tier-F self-check** (Form D did). Closed it — `asm-formc-raw-xml` + `asm-formc-fields` (verifiable, live-checked against a reference Form C) so an EDGAR schema/localname change drifts loudly instead of silently flipping `cf_cap` / `cf_single_intermediary` / `cf_state_notice` / the issuer-side bad-actor screen. Plus consistency fixes: registered the `cf_form_c_filed` existence-only timing limitation as a first-class **accepted** caveat (`asm-formc-filing-timing`, parity with the bad-actor rolling-window caveat — now renders on the panel); clarified the assumption `'*'` wildcard scope (Form-D-only); de-506(b)-ified the shared `control-panel.py` docstring/help (it is multi-framework via `--framework`); documented the one-fact-escalates-three-sovereigns coupling as intentional (cumulative duties, cross-referenced, not de-duplicated). Tier-F registry now **17 nodes, 0 violated**; `make test` green (13 suites); panel clean on live data (8 open / 1 satisfied / 0 conclusions / 0 coverage gaps). Audit retrospective in `docs/product-thesis.md` v0.20 + `docs/authority-model.md` v0.7.
...
- **State-notice leg refined — `cf_state_notice` states the Section 18 preemption as public law** — `cf_state_notice` (state blue-sky) moves from `manual_private` to a `cf_state_notice` evaluator that surfaces the load-bearing **public legal fact**: Reg CF securities are "covered securities" under 15 U.S.C. 77r(b)(4)(C) (§4(a)(6)), so state **registration / qualification is preempted**. It also lists the offering's jurisdictions from the Form C (`issueJurisdictionSecuritiesOffering`), reducing the state surface to what actually remains — residual, state-specific **notice filings** (limited for Reg CF, not on EDGAR) + the states' preserved **antifraud** authority — which it routes to counsel. Careful never-opine framing: it states the *statute* (the covered-securities designation), explicitly NOT a conclusion about this offering's state compliance. Verified live on 20Slash20, Inc. (52 jurisdictions listed; preemption stated; residual notices left to counsel). New hermetic tests (preemption stated, jurisdictions listed, no-jurisdictions fallback, never-opine); `make test` green (13 suites). With this, the **only fully-manual control left in the funding-portal set is `portal_revenue_report`** — a FINRA-internal Form FP-Statement of Revenue that is genuinely not public.
- **Single-intermediary cross-check — `cf_single_intermediary` goes from blind to a public-record test** — `cf_single_intermediary` (17 CFR 227.300(a),(b): an offering must be conducted exclusively through ONE registered intermediary) moves from `manual_private` to a deterministic EDGAR cross-check (`edgar_formd.list_form_c_intermediaries` + a `cf_single_intermediary` evaluator). It scans the issuer's own Form C / C-A filings, extracts the named intermediary from each, and reasons with the **time-window precedent** from the Form D integration check: a single named intermediary across the record → `open` (evidenced, consistent with one-intermediary); distinct intermediaries over **overlapping** offering windows (`filingDate`→`deadlineDate`) → `escalate` (a single offering may have used more than one intermediary, the 227.300(b) concern); distinct intermediaries in **non-overlapping** periods → `open` (permitted sequential raises). A missing deadline is treated as open-ended (conservatively overlaps → escalate-safe). Because this is exemption-fatal, an **incomplete scan never clears to a clean single-intermediary record** (the key never-opine guard): truncation (older filings unscanned), a parse gap (a Form C that didn't parse — and a failed *fetch* `SystemExit`s, so it's caught and dropped, not crashed), or an unresolved intermediary all downgrade the note to "NOT a full-record confirmation"; if no intermediary resolves at all it says "could not identify," never "single intermediary." Intermediary identity is keyed CIK → file-number → name (so a distinct portal with an unparsed CIK isn't silently merged), and the raw-XML basename is derived from `primaryDocument` (not hardcoded). Exclusivity also depends on off-platform conduct, which stays private — so it never reaches `satisfied`. Verified live on 20Slash20, Inc. (one Form C → Mr. Crowd → evidenced open). New hermetic tests for the overlap detection, every evaluator branch, the incompleteness guards (truncation / parse-gap / unresolved / failed-fetch-skip), the distinct-by-file-number path, and the never-opine invariant; `make test` green (13 suites). (Verifier pass folded in: it confirmed and I closed four false-clearance vectors that all shared one root — rows silently excluded from the cross-check must surface, not clear.)
- **Portal prohibited-conduct evidenced — and a never-opine discipline test passed** — `portal_prohibited_conduct` (17 CFR 227.300(c)/402 + FINRA FP Rule 200) moves from blind `manual_private` to a `portal_conduct` evaluator that assembles public CFPORTAL evidence (reusing the same Form Funding Portal already fetched for the fund-custody leg — no extra fetch): the **funds-handling prong** is cross-referenced to `funds_qualified_third_party` (portal routes funds to a third party, not itself), and the portal's **structured criminal/regulatory/civil/financial disclosure answers** are scanned (scoped precisely to the negative-event subtrees, excluding neutral structural fields) → `escalate` on any affirmative event (FP Rule 200 standing), else evidenced. Critically, the **disclosed compensation is surfaced but NOT flagged**: a funding portal may lawfully receive transaction-based compensation and take a financial interest in an issuer (227.205/300(c)), so flagging it would be the system drawing a wrong legal conclusion — the evaluator explicitly states permissibility is counsel's call, not the system's. The no-advice/no-solicitation prongs stay private. Resolves to `open` (evidenced) for a clean portal; verified live on Mr. Crowd / Ksdaq Inc. (all disclosures negative, funds → North Capital). The disclosure scan is **prefix-agnostic** (flags any affirmative leaf in the negative-event subtrees, so a field that breaks the `is`/`has`/`does` naming convention can't be silently missed → no false "all-negative" clearance), registered as a verifiable system-assumption (`asm-cfportal-disclosure-fields`, live-checked; registry now 14). New hermetic tests for the disclosure scan (neutral-field exclusion, `has`-prefix + non-conventional field names, subtree-presence count), the evaluator's never-opine compensation handling, and the withdrawn-portal cause-attribution; `make test` green (13 suites).
- **Money-transmission leg auto-evidenced — `funds_qualified_third_party` goes from blind to verified** — `scripts/fund-custody-check.py` evidences the fund-custody / money-transmission control (17 CFR 227.303(e)) end-to-end from public EDGAR data: (1) the portal's own **Form Funding Portal (`CFPORTAL`)** discloses, in structured fields (`escrowArrangements` → `investorFundsContacts/investorFundsContactName`), the entity that holds investor funds — which the check confirms is a **third party distinct from the portal** (if the portal names itself → `escalate`: direct money-transmitter / FinCEN MSB exposure); (2) the named custodian's status as a "qualified third party" is itself verified — a registered broker-dealer files annual **`X-17A-5` FOCUS reports** on EDGAR, so the check resolves the custodian name → CIK (EDGAR atom company search) and confirms X-17A-5 in its history (a bank custodian, equally qualified, won't file X-17A-5 → "confirm manually", not a failure). Resolves to `open` (heavily evidenced) when the 227.303(e) structure checks out — **not `satisfied`**, because the executed escrow agreement for the specific offering + the AML/BSA program stay private (crisply delineated). Verified live on Mr. Crowd / Ksdaq Inc.: funds → **North Capital Private Securities Corporation** (CIK 1496269, a registered BD), distinct from the portal. Wired into `control-panel.py` via `mtl_fund_custody` (control locus `private` → `hybrid`). New verifiable system-assumption (`asm-cfportal-escrow-fields`, live-checked, holds; registry now 13) + hermetic `tests/test_fund_custody.py` (canned CFPORTAL XML + mocked registers, every verdict branch + never-opine invariant). This is the **first money-transmission-sovereign control to reach an evidenced state**; `make test` green (13 suites). Method captured in `CLAUDE.md`.
- **FINRA membership auto-verification — `portal_finra_member` goes live** — `scripts/finra-portal-check.py` verifies a Reg CF intermediary is a **registered funding portal AND a current FINRA member** (17 CFR 227.400 + FINRA FP Rule 110) from the two authoritative public registers, joined on the SEC file number: (1) **SEC registration** via EDGAR's `CFPORTAL` form family (`CFPORTAL` / `CFPORTAL/A` / `CFPORTAL-W` withdrawal) — registered = a CFPORTAL exists and the latest is not a `-W`; (2) **FINRA membership** via the "Funding Portals We Regulate" page (plain HTML, directly `curl`-able — `sec-fetch.sh` is sec.gov-only so FINRA uses a direct fetch), matched on `filenum=` (Form C's `007-00042` → FINRA's `7-42`, stripping leading-zero padding per part). Resolves to `satisfied` only when both registers confirm with reconciling legal names (a pure public-register fact); `escalate_to_counsel` on withdrawal / register inconsistency / name mismatch; `open` if a register is unreachable (never a silent clearance). Wired into `control-panel.py` via `cf_intermediary` (computed once in `build()`, like the bad-actor screen). This is the **first FINRA-sovereign control to reach a verified state**, and it deterministically ties the Form C brand name ("Mr. Crowd") to the legal filer ("Ksdaq Inc."). Two new **system-assumption meta-nodes** registered (`asm-cfportal-forms`, `asm-finra-fp-html`, both with live checks that hold) + one accepted fail-safe (`asm-fileno-normalize`); `controls/assumptions.json` now 12 nodes. New hermetic `tests/test_finra_portal.py` (canned FINRA HTML + mocked registers) covers normalization, deterministic parsing, and every verdict branch incl. the never-opine invariant. Verified live on Mr. Crowd / Ksdaq Inc. (file `007-00042`); `make test` green (12 suites). Method captured in `CLAUDE.md`.
- **Form C driver — Regulation Crowdfunding / funding-portal surface goes live** — `scripts/edgar_formd.py` now drives **both** Form D and Form C (`load_form_c` / `parse_form_c`, same envelope shape as `load_form_d`); `control-panel.py` dispatches on a framework's new `driver` field (`form_c` vs default `form_d`). This turns `controls/reg-cf-funding-portal.json` from a stub into a live multi-sovereign panel: a real Form C supplies the public-side legs — Form C existence on EDGAR (`cf_form_c_filed`, which stays `open`, not `satisfied`: the obligation is multi-part — filed *before commencement* + annual C-AR — and only existence is verified, mirroring the Form D `form_d_filed` precedent), offering maximum vs the $5M **rolling 12-month** Reg CF ceiling (`cf_cap`, stays `open` since an aggregate can't be concluded from one filing), the single named intermediary by CIK + CRD + `007-` funding-portal file number (`cf_intermediary`, feeding the FINRA-membership check), and the signature persons fed into the 227.503 bad-actor screen — while the FINRA-conduct, fund-custody, and state legs resolve to `open`/`escalate`. The panel draws **0 legal conclusions** (every leg routes to counsel or stays open; control *states* like `satisfied` are evidence-states from public registers, not legal conclusions). Exercises the **FINRA + money-transmission** sovereigns end-to-end (0 coverage gaps). Verified live on CIK 2140631 (20Slash20, Inc. via the Mr. Crowd portal); 506(b)/506(c) Form D path regression-clean; `make test` green. Gotcha captured: Form C's `primaryDocument` is the XSLT HTML `xslC_X01/primary_doc.xml` (raw XML is the prefix-stripped sibling, same pattern as Form D).
- **Rule 506(c) control set** — `controls/reg-d-506c.json` (15 controls) adds a second offering type, proving the framework is a platform: same authority registry, sovereign axis, drift pins, counsel node, and assumption checks — just a new control set. 506(c)-specific controls (all-purchasers-accredited; reasonable-steps-to-verify; verification-method-documented) with two new auto-evaluators (`exemption_is_506c`, `all_accredited_506c`). Verified live on a real 506(c) filer.
- **System-assumption meta-node (authority model)** — `controls/assumptions.json` + `scripts/assumption-check.py` register the truths the evaluators rely on: **verifiable** assumptions with executable checks (Form D raw-XML location, field localnames, the `06b` code, eCFR/Federal-Register/AP-RSS shapes) and **accepted** limitations (substring bad-actor matching, AP rolling-window, the 180-day integration proxy). The checker runs verifiable assumptions against live data (6/6 hold) and, on a VIOLATED check, flags the controls that assumption could silently corrupt. `control-panel.py` surfaces accepted limitations on the controls that rely on them. Completes all six tiers of the authority model.
- **Federal Register drift source (authority model)** — `scripts/fedreg-watch.py` closes the "binding ≠ codified" gap that an eCFR diff misses. Queries the Federal Register API for SEC documents since a date and maps each to authority nodes by CFR part: `AFFECTS-NODES` (touches a part we cite → lists dependent controls for review), `OTHER-SEC-RULE` (title 17, other parts → context), `UNMAPPED-REVIEW` (no CFR reference, e.g. an exemptive order → the binding-but-uncodified catch). Prints a truncation note past the page limit; never concludes ("review; counsel assesses").
- **Named-counsel terminal node (authority model)** — `scripts/counsel.py` + `opinions/*.json`. A fresh opinion-of-record closes covered controls into `satisfied_by_counsel`, attributed to a named attorney + date + opinion id (the system still draws 0 conclusions). The opinion snapshots the authority versions and facts it relied on and **decays**: `control-panel.py --opinions` reverts a control to `escalate_to_counsel` ("STALE — re-opine") on law-drift (a cited authority's pinned_version moved) or fact-drift (the relied-on Form D was superseded). Verified end-to-end. `opinions/eccentric-machines.json` is a clearly-labeled synthetic sample.
- **Provenance-as-edge + drift (authority model)** — `controls/authorities.json` is the authority registry (typed nodes with `pinned_version`; CFR sections pinned to current eCFR amendment_dates). Controls cite authorities by id via `authority_refs` (the provenance edge; `scripts/authority_registry.py` resolves them). `scripts/authority-drift.py` checks each eCFR-sourced node against current eCFR and propagates a stale-flag to every dependent control on a version change (verified: un-pinning §230.506 flags all 4 citing controls `stale → re-verify → escalate`, never "non-compliant"). `control-panel.py` displays the pinned version per control.
- **Sovereign axis (authority model)** — every control in `controls/*.json` now carries a `sovereign` (federal-sec / finra / state-blue-sky / money-transmission / corporate) and each framework declares `sovereigns_in_scope`. `control-panel.py` self-reports **sovereign coverage** and flags any in-scope sovereign with 0 controls as a `COVERAGE GAP → escalate` (so the panel can never look complete while silently under-covering). Adding the axis exposed and closed a real corporate-authorization gap in the 506(b) set. Added `controls/reg-cf-funding-portal.json` (multi-sovereign stub for the funding-portal surface) and a `--framework` flag. Design: `docs/authority-model.md`.
- **`controls/reg-d-506b.json` + `scripts/control-panel.py` + `scripts/edgar_formd.py`** — the first counsel-ready control panel. The §506(b) control framework as data (14 atoms from 17 CFR 230.501–503 / 506(b)(d)(e), each with authority + the five axes: state/locus/owner/cadence/severity). `control-panel.py` loads a CIK's Form D, auto-computes control state where public data allows (exemption claimed, non-accredited ceiling, info-delivery N/A, bad-actor screen, **integration window** vs the issuer's other EDGAR offerings, amount + first-sale extraction), marks the rest open/private, and renders the full checklist with an "exemption-fatal, not yet satisfied" urgent view — never-opine. `--html` emits a self-contained counsel-ready report (badged states, urgent box, "draws no legal conclusions" banner, citations + axes). Shared Form D access/parse + filing-history in the importable `edgar_formd.py` (screen-offering.py refactored onto it; `make test` green).
